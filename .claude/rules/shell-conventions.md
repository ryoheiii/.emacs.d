...
- **`pipefail` 下で `grep -q` や `head` のような早期終了コマンドへパイプしない。**
  読み手が先に終了すると書き手が SIGPIPE で 141 になり、判定が成立していても
  パイプライン全体が失敗する。出力を変数へ受けてから `[[ $var = pat* ]]` や
  `case` で判定する（`apt-cache policy … | grep -q` が実在するパッケージを
  「見つかりません」へ倒していた）。
