...
ここで導入されるパッケージの選択は、`make test-setup` が apt をスタブに差し替えて
検証している（実際の `apt-get` は起動しない）。
...
**`--install` は自動テストの対象外である。** 実ビルドを伴うため CI でも
`make test-setup` でも実行していない。とくに `--gui no`（`--without-x`）での
ビルドは未検証であり、端末専用環境で使う場合は手元で結果を確認してほしい。
